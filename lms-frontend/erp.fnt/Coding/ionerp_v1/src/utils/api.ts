...
axiosInstance.interceptors.response.use(
  ...
  (error) => {
    ...
    const now = Date.now();
    const errorKey = error.response?.status ? `http_${error.response.status}` : 'network';
    const lastToastTime = (window as any).lastToastTime?.[errorKey] || 0;
    
    if (now - lastToastTime < 3000) { // 3s debounce per error type
    ...
    if (error.response) {
      ...
      if (status === 404) {
        if (optionalEndpoints.some(endpoint => requestUrl.includes(endpoint))) {
          console.warn(`⚠️ Optional endpoint 404 (suppressed): ${requestUrl}`);
          return Promise.reject(error); // Fail silently for dropdowns
        }
        toast.error(error.response.data?.message || `Resource not found: ${requestUrl.split('?')[0]}`);
        return Promise.reject(error);
      }
      ...
      if (networkToastCount === 0) {
        toast.error("Network error: Server is unreachable. Check if backend is running.");
        (window as any).networkToastCount = 1;
      } else {
        console.error("🌐 Network unreachable (toast suppressed after 1st):", error.message);
      }
    }
