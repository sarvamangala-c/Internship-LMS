...
const doesSectionOptionMatch = (option: TimetableOptionItem, sectionValue: string) => {
...
const resolveSemesterFromTerm = (termId: string, rawTerm?: any) => {
  const explicitSemester = Number(rawTerm?.semester ?? rawTerm?.term_name ?? rawTerm?.semester_id);

  if (Number.isFinite(explicitSemester) && explicitSemester > 0 && explicitSemester < 100) {
...
const getFirstPlanDateForDay = (rangeStart: string, rangeEnd: string, dayName: DayName) => {
...
const TimetableListPage: React.FC = () => {
  ...
  const [timetables, setTimetables] = useState<TimetableOptionItem[]>([NEW_TIMETABLE_OPTION]);
  const [scheduledClasses, setScheduledClasses] = useState<any[]>([]);

  const [selectedCurriculum, setSelectedCurriculum] = useState("");
  const [selectedTerm, setSelectedTerm] = useState("");
  const [selectedSection, setSelectedSection] = useState("");
  const [selectedSectionState, setSelectedSectionState] = useState<SelectedSectionState>(EMPTY_SELECTED_SECTION);
  ...
  const selectedSectionOption = sections.find((section) =>
  ...
  const selectedTermRequestId = getNumericId(selectedCrclmTermId, selectedSemesterId, selectedTerm) ?? 0;
  ...
  const selectedTermLabel =
    ...
    selectedTerm;
  const resolvedTermName = selectedSemesterId || resolveSemesterFromTerm(selectedTerm, selectedTermOption?.raw);
  ...
  const scheduledClassesQuery = useMemo(
    () => ({
      ...
      termId: selectedTermRequestId || undefined,
      sectionId: selectedSectionRequestValue,
      startDate: startDate || undefined,
      endDate: endDate || undefined,
    }),
    [endDate, selectedCurriculumId, selectedSectionRequestValue, selectedTermRequestId, startDate],
  );

  const syncSelectedSectionState = (sectionValue: string, options: TimetableOptionItem[]) => {
    const matchedOption = options.find((option) => doesSectionOptionMatch(option, sectionValue));
  ...
  useEffect(() => {
    console.log("[TimetablePage] Selected section:", selectedSectionValue ?? "<empty>");
  }, [selectedSectionValue]);
  ...
  useEffect(() => {
    if (!selectedCurriculum || !selectedTerm) {
      console.log("[TimetablePage] resetting section", {
        reason: !selectedCurriculum ? "curriculum missing" : "term changed or cleared",
        selectedSection,
        nextOptions: [],
      });
      setSections([]);
    ...
    const loadSections = async () => {
      ...
      const response = await timetableApi.getSections(Number(selectedCurriculum), resolvedTermName);
      const sectionOptions = (response.data as TimetableOptionItem[]) || [];
      console.log("[TimetablePage] transformed section options", sectionOptions);
      ...
      const hasMatchingSection = sectionOptions.some((option) => doesSectionOptionMatch(option, selectedSection));
      ...
      const selectedTermOption = terms.find((term) => term.value === selectedTerm)?.raw;
      if (!startDate) {
        setStartDate(
  ...
  useEffect(() => {
    ...
    const loadTimetableOptions = async () => {
      ...
      console.log("[TimetablePage] raw batch/sections API response request payload", payload);
  ...
  useEffect(() => {
    const loadScheduledClasses = async () => {
      ...
      if (missingFilters.length > 0) {
        console.log("[TimetablePage] skipping scheduled classes fetch; missing required filters", {
          missingFilters,
          selectedCurriculum,
          selectedTerm,
          selectedSection,
      ...
      if (selectedSection && !selectedSectionValue) {
        console.warn("[TimetablePage] Blocking scheduled classes fetch because section selection is invalid.");
        setIsPageLoading(false);
        return;
      }

      setIsPageLoading(true);
      const response = await timetableApi.getScheduledClasses(scheduledClassesQuery);
      const nextScheduledClasses = Array.isArray(response.data) ? response.data : [];
      console.log("[TimetablePage] raw scheduled-classes response", nextScheduledClasses);
  ...
  const filteredClasses = useMemo(() => {
    const normalizedSelected = String(selectedSection || "").trim().toUpperCase();
    console.log("Selected section:", selectedSection);

    const filtered = scheduledClasses.filter((item) => {
      console.log("Item section:", item?.section);

      const itemSection = String(item?.section || item?.section_id || item?.sectionId || "")
        .trim()
        .toUpperCase();

      return !normalizedSelected || itemSection === normalizedSelected;
    });

    console.log("Filtered data:", filtered);
    return filtered;
  }, [scheduledClasses, selectedSection]);
  ...
  const transformedGridData = useMemo(() => {
    return filteredClasses
      ...
      .filter(Boolean) as Array<any & { day: DayName; timeLabel: string; displayCode: string }>;
  }, [filteredClasses]);
  ...
  const classesByCell = useMemo(() => {
    ...
    transformedGridData.forEach((item) => {
  ...
  const selectedTimetableOption = timetables.find((item) => item.value === selectedTimetable);
  ...
  const handleScheduleSave = async (modalData: {
    ...
    courseId: string;
    courseLabel: string;
    days: Array<{ name: DayName; selected: boolean; startTime: string; endTime: string }>;
  }) => {
    if (!selectedCurriculumId || !selectedTermRequestId || !selectedSectionLabel || !startDate || !endDate) {
      toast.error("Curriculum, Term, Section, Start Date, and End Date are required.");
      return;
    }

    console.log("[TimetablePage] selected section object before save", selectedSectionState);
    ...
    if (invalidRow) {
      toast.error(`No plan date found within the selected range for ${invalidRow.day}.`);
      return;
    }

    console.log("[TimetablePage] schedule-class payload before fix", payloadRowsBeforeFix);
    ...
    if (result?.success) {
      ...
      const refreshed = await timetableApi.getScheduledClasses(scheduledClassesQuery);
      setScheduledClasses(Array.isArray(refreshed.data) ? refreshed.data : []);
    }
  };

  const handleOptionsAction = async (action: "import" | "export" | "copy" | "reset" | "delete") => {
    setIsOptionsOpen(false);

    if (action === "export") {
      if (!selectedCurriculum || !selectedTerm || !selectedSectionValue) {
        console.warn("[TimetablePage] Blocking export because section selection is missing or invalid.", {
          selectedCurriculum,
          selectedTerm,
          selectedSection,
        });
        toast.info("Select curriculum, term, and section before exporting.");
        return;
      }

      const exportPayload = {
        semTimeTableId: selectedTimetable === "new" ? undefined : Number(selectedTimetable),
        ...scheduledClassesQuery,
      };
    ...
    toast.info("This option is shown to match the sir page and will be enabled when backend wiring is ready.");
  };
    ...
    <div style={pageStyle}>
      <div style={cardStyle}>
          ...
          <div style={optionsWrapperStyle}>
            ...
            {isOptionsOpen && (
              <div style={dropdownStyle}>
                <OptionItem label="Import Timetable" disabled onClick={() => handleOptionsAction("import")} />
                <OptionItem
                  label="Export Timetable"
                  disabled={!selectedCurriculum || !selectedTerm}
                  onClick={() => handleOptionsAction("export")}
                />
                <OptionItem label="Copy Class Day" disabled onClick={() => handleOptionsAction("copy")} />
                <OptionItem label="Reset Date" disabled onClick={() => handleOptionsAction("reset")} />
                <OptionItem label="Delete" disabled onClick={() => handleOptionsAction("delete")} danger />
              </div>
            )}
          </div>
        </div>
        ...
        <div style={gridShellStyle}>
          ...
          <div style={tableScrollStyle}>
            <table style={tableStyle}>
              ...
              <tbody>
                {timeSlots.map((slot) => (
                  <tr key={slot}>
                    <td style={{ ...bodyCellStyle, ...timeColumnStyle }}>{formatTimeToAMPM(slot)}</td>
                    {DAY_NAMES.map((day) => {
                      const timeLabel = formatTimeToAMPM(slot);
                      const cellItems = classesByCell.get(`${day}|${timeLabel}`) || [];
                      return (
                        <td key={`${day}-${slot}`} style={bodyCellStyle}>
                          {cellItems.length === 0 ? (
                            ...
                            cellItems.map((item, index) => (
                              <div key={`${day}-${slot}-${index}`} style={classChipStyle}>
                                <div style={classTitleStyle}>
                                  {item?.displayCode}
                                </div>
                                <div style={classSubtitleStyle}>
                                  {item?.course_type || item?.courseType || item?.faculty || ""}
                                </div>
                                <div style={classTimeStyle}>
                                  {item?.timeLabel}
                                  {item?.end_time || item?.endTime ? ` - ${formatTimeRangeValue(item?.end_time || item?.endTime)}` : ""}
                                </div>
                              </div>
                            ))
                          )}
                ...
                {!isPageLoading && timeSlots.length === 0 && (
                  <tr>
                    <td style={emptyStateCellStyle} colSpan={8}>
                      Choose a valid start and end time to render the timetable grid.
                    </td>
                  </tr>
                )}
              </tbody>
          ...
          {isPageLoading && <div style={statusLineStyle}>Loading timetable...</div>}
          {!isPageLoading && transformedGridData.length === 0 && (
            <div style={statusLineStyle}>No scheduled classes found for the current selection.</div>
          )}
        </div>
      </div>
...
const TimeField: React.FC<{
  ...
  <div style={fieldWrapStyle}>
    <label style={labelStyle}>{label}</label>
    <input type="time" value={value} onChange={(event) => onChange(event.target.value)} style={inputStyle} />
  </div>
);
...
const getDayNameForRecord = (item: any): DayName | null =>
  normalizeDay(item?.day || item?.day_name || item?.weekday) || getDayNameFromDate(item?.date);
...
const optionItemStyle = (disabled?: boolean, danger?: boolean): React.CSSProperties => ({
  width: "100%",
  textAlign: "left",
  border: "none",
  backgroundColor: "#ffffff",
