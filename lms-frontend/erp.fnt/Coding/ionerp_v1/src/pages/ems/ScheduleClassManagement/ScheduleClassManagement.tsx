...
const ScheduleClassManagement: React.FC = () => {
  ...
  const generateFingerprint = useCallback((item: any) => {
    ...
    const topic = normalize(
      item.topic ||
        (item.topicId ? getTopicNameById(Number(item.topicId)) : ""),
    );
  ...
  const transformClassData = useCallback(
    (item: any): ScheduledClass => {
      ...
      return {
        ...
        time:
          ...
          (item.startTime && item.endTime
            ...
            : "-"),
        courseType:
          item.courseType || (item.courseTypeId === 1 ? "Theory" : "Practical"),
        course:
          item.course ||
          (item.courseId ? getCourseNameById(Number(item.courseId)) : "-"),
        section:
  ...
  const loadScheduledClasses = useCallback(async () => {
    try {
      ...
      console.log(
        `[Sync] Found ${formattedData.length} total entries, showing ${finalResult.length} unique records.`,
      );
    ...
    () => [
      ...
      },
      {
        ...
        cellRenderer: (params: any) => {
          ...
          const color = isTheory
            ? "bg-blue-50 text-blue-700"
            : "bg-green-50 text-green-700";
          return (
            <div
              className={`px-2 py-0.5 text-[10px] font-bold uppercase rounded border inline-block ${color}`}
            >
              {type}
            </div>
          );
        ...
        cellRenderer: (params: any) => {
          ...
          return (
            <button
              ...
              className="text-red-500 hover:text-red-700 font-bold text-[10px] uppercase underline cursor-pointer"
            >
              [REMOVE]
            </button>
          );
  ...
  return (
    <div className="p-6 bg-[#f8fafc] min-h-screen">
      <div className="flex justify-between items-center mb-6 bg-white p-6 rounded-lg border shadow-sm">
        <div>
          <h1 className="text-xl font-bold text-[#1f4e5f] uppercase tracking-tighter">
            Scheduled Classes
          </h1>
        </div>
      ...
      <div className="bg-white rounded-lg border shadow-md overflow-hidden">
        {loading ? (
          <div className="py-20 text-center font-black text-slate-300 uppercase text-[9px] tracking-[4px]">
            Synchronizing...
          </div>
        ) : (
          <DataTable
