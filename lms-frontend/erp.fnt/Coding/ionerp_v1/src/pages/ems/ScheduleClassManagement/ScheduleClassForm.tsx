...
import {
  validateScheduleClass,
  hasValidationErrors,
} from "../../../schemas/scheduleClassSchema";
...
const ScheduleClassForm: React.FC<ScheduleClassFormProps> = ({
  ...
  useEffect(() => {
    ...
    const topics =
      topicsByCourse[
        Number(formData.courseId) as keyof typeof topicsByCourse
      ] || [];
  ...
  useEffect(() => {
    ...
    const topic = availableTopics.find(
      (t) => t.id === Number(formData.topicId),
    );
    if (topic) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(
        error.message || "Failed to schedule class. Please try again.",
      );
    } finally {
      setLoading(false);
    }
