...
# ============================================================
# REVIEW: CUDOS TOPIC MODEL - Master topic catalog
# Table: cudos_topic
# Purpose: Central source for all topic data → imported to LMS tables
# Used in: Topic management APIs (import/list/update flows)
from sqlalchemy import UniqueConstraint

class CudosTopic(Base):
    __tablename__ = "cudos_topic"

    topic_id = Column(Integer, primary_key=True, index=True)  # REVIEW: Auto-increment PK
    topic_code = Column(String(10))  # REVIEW: Should be unique per course/semester?
    topic_title = Column(String(500))
    topic_content = Column(Text)
    topic_hrs = Column(String(8))  # REVIEW: String → Float/Duration type?
    num_of_sessions = Column(Float)
    marks_expt = Column(Float)  # REVIEW: Expected marks per topic
    correlation_with_theory = Column(String(200))
    conduction_date = Column(Date)  # REVIEW: Planned vs actual?
    ...
    __table_args__ = (
        UniqueConstraint('topic_code', 'course_id', 'semester_id', 'academic_batch_id', name='uq_topic_uniqueness'),
    )

    # TODO: Add relationships
    # instructors = relationship("LMSMapInstructorTopic", back_populates="topic")
    # schedules = relationship("TopicLessonSchedule", back_populates="topic")
