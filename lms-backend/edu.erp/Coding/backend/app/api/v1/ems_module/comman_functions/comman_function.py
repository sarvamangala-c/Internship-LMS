...
class CourseRequest(BaseModel):
    academic_batch_id: int = Field(
        ...,
        description="Curriculum id. This maps to iems_academic_batch.academic_batch_id.",
        example=1,
    )
    semester: Optional[int] = Field(
        default=None,
        description="Accepted values: semester number, iems_semester.semester_id, or UI crclm_term_id.",
        example=101,
    )
    semester_id: Optional[int] = Field(
        default=None,
        description="Alias-style input for term selection. Accepts semester number, iems_semester.semester_id, or UI crclm_term_id.",
        example=101,
    )
    crclm_term_id: Optional[int] = Field(
        default=None,
        description="Optional explicit UI term id from iems_crclm_term.crclm_term_id.",
        example=101,
    )
    course_type_id: Optional[int] = Field(
        default=None,
        description="Optional course type filter from iems_course_type.course_type_id.",
        example=1,
    )
...
class ScheduleRequest(BaseModel):
    ...
    semester_id: int = Field(
        ...,
        description="Term input. Accepts UI crclm_term_id, iems_semester.semester_id, or plain semester number.",
        example=101,
    )
    crclm_term_id: Optional[int] = Field(
        default=None,
    ...
    crs_id: int = Field(
        ...,
        description="Course id. This maps to iems_courses.crs_id.",
        example=1,
    )
    section_id: int = Field(
        ...,
        description="Section id. This maps to iems_section.id. No separate batch id is required in save payload.",
        example=1,
    )
    plan_date: date = Field(..., description="Scheduled class date.", example="2026-04-10")
    start_time: str = Field(..., description="Start time in HH:MM or HH:MM:SS.", example="09:00")
    end_time: str = Field(..., description="End time in HH:MM or HH:MM:SS.", example="10:00")
    created_by: int = Field(..., description="User id creating the schedule row.", example=1)
...
class DuplicateRequest(BaseModel):
    ...
    section_id: int = Field(
        ...,
        description="Section id. This maps to iems_section.id.",
        example=1,
    )
    plan_date: date = Field(..., description="Scheduled class date.", example="2026-04-10")
    start_time: str = Field(..., description="Start time in HH:MM or HH:MM:SS.", example="09:00")
...
def _request_to_dict(instance: BaseModel) -> Dict:
...
def _normalize_time_value(value: Optional[str]) -> Optional[str]:
    ...
    raise HTTPException(
        status_code=422,
        detail={
            "message": f"Invalid time value '{value}'. Expected HH:MM or HH:MM:SS.",
            "field": "time",
        },
    )
...
def _resolve_semester_context(
    ...
    context: Dict[str, Optional[int | str]] = {
    ...
    term_query = db.query(IEMSCrclmTerm)
    semester_query = db.query(IEMSemester)
    if academic_batch_id is not None:
        term_query = term_query.filter(IEMSCrclmTerm.crclm_id == academic_batch_id)
        semester_query = semester_query.filter(IEMSemester.academic_batch_id == academic_batch_id)

    resolved_term = term_query.filter(IEMSCrclmTerm.crclm_term_id == requested_value).first()
    if resolved_term is not None:
        context["resolved_crclm_term_id"] = resolved_term.crclm_term_id
        context["resolved_semester_number"] = resolved_term.term_name
        context["resolution_source"] = "crclm_term_id"
    else:
        resolved_semester = semester_query.filter(IEMSemester.semester_id == requested_value).first()
        if resolved_semester is not None:
            context["resolved_semester_id"] = resolved_semester.semester_id
            context["resolved_semester_number"] = resolved_semester.semester or resolved_semester.semester_id
            context["resolution_source"] = "semester_id"
        else:
            resolved_semester = semester_query.filter(IEMSemester.semester == requested_value).first()
            if resolved_semester is not None:
                context["resolved_semester_id"] = resolved_semester.semester_id
                context["resolved_semester_number"] = resolved_semester.semester or requested_value
                context["resolution_source"] = "semester_number"
            else:
                context["resolved_semester_id"] = requested_value
                context["resolved_semester_number"] = requested_value
                context["resolution_source"] = "raw"

    if context["resolved_semester_id"] is None and context["resolved_semester_number"] is not None:
        semester_row = semester_query.filter(
            IEMSemester.semester == context["resolved_semester_number"]
        ).first()
        if semester_row is not None:
            context["resolved_semester_id"] = semester_row.semester_id

    if context["resolved_crclm_term_id"] is None and academic_batch_id is not None and context["resolved_semester_number"] is not None:
        term_row = db.query(IEMSCrclmTerm).filter(
            IEMSCrclmTerm.crclm_id == academic_batch_id,
            IEMSCrclmTerm.term_name == context["resolved_semester_number"],
        ).first()
...
def list_courses(request: CourseRequest, db: Session = Depends(get_db)):
    """List courses for a curriculum and resolved term.
    ...
    return {
        "status": True,
        "resolved_semester_id": semester_context["resolved_semester_id"],
        "resolved_semester_number": semester_context["resolved_semester_number"],
        "resolved_crclm_term_id": semester_context["resolved_crclm_term_id"],
        "data": [
            {
                key: value
...
def list_batch_sections(request: BatchSectionRequest, db: Session = Depends(get_db)):
    """List sections for a curriculum and resolved term.
    ...
    sections = db.query(IEMSection).filter(
        ...
        or_(
            IEMSection.semester_id == semester_context["resolved_semester_id"],
    ...
    return {
        ...
        "resolved_semester_number": semester_context["resolved_semester_number"],
        "resolved_crclm_term_id": semester_context["resolved_crclm_term_id"],
        "batch": _clean_model_dict(batch) if batch else None,
        "sections": [_clean_model_dict(row) for row in sections]
    }


def save_schedule(request: ScheduleRequest, db: Session = Depends(get_db)):
    """Create a row in lms_lesson_schedule.

    Required frontend payload:
    - academic_batch_id = curriculum
    - semester_id = term / crclm_term_id accepted
    - crs_id = course
    - section_id = section (iems_section.id)
    """
    request_payload = _request_to_dict(request)
    semester_context = _resolve_semester_context(
        db,
        request.academic_batch_id,
        request.crclm_term_id or request.semester_id,
    )
    normalized_start_time = _normalize_time_value(request.start_time)
    normalized_end_time = _normalize_time_value(request.end_time)

    logger.warning("schedule-class incoming payload=%s", request_payload)
    logger.warning("schedule-class resolved semester=%s", semester_context)
    ...
    if batch is None:
        raise HTTPException(
            status_code=404,
            detail={
                "message": "Invalid academic_batch_id. Curriculum not found.",
                "field": "academic_batch_id",
                "value": request.academic_batch_id,
            },
        )

    course_query = db.query(IEMSCourses).filter(
        IEMSCourses.crs_id == request.crs_id,
        IEMSCourses.academic_batch_id == request.academic_batch_id,
    )
    if semester_context["resolved_semester_number"] is not None and hasattr(IEMSCourses, "semester"):
        course_query = course_query.filter(IEMSCourses.semester == semester_context["resolved_semester_number"])
    course = course_query.first()
    if course is None:
        raise HTTPException(
            status_code=404,
            detail={
                "message": "Invalid crs_id for the provided curriculum/term.",
                "field": "crs_id",
                "value": request.crs_id,
                "resolved_semester_number": semester_context["resolved_semester_number"],
            },
        )

    section = db.query(IEMSection).filter(
        IEMSection.id == request.section_id,
        IEMSection.academic_batch_id == request.academic_batch_id,
        or_(
            IEMSection.semester_id == semester_context["resolved_semester_id"],
            IEMSection.semester_id == semester_context["resolved_semester_number"],
    ...
    logger.warning(
        "schedule-class resolved section_id=%s section=%s",
        getattr(section, "id", None),
        getattr(section, "section", None),
    )
    if section is None:
        raise HTTPException(
            status_code=404,
            detail={
                "message": "Invalid section_id for the provided curriculum/term.",
                "field": "section_id",
    ...
    duplicate = db.query(LMSLessonSchedule).filter(
        ...
        LMSLessonSchedule.semester_id == semester_context["resolved_semester_id"],
        LMSLessonSchedule.section_id == request.section_id,
        LMSLessonSchedule.plan_date == request.plan_date,
        LMSLessonSchedule.start_time == normalized_start_time,
        LMSLessonSchedule.end_time == normalized_end_time
    ).first()
    logger.warning(
        "schedule-class duplicate result=%s",
        _clean_model_dict(duplicate) if duplicate else None,
    )
    ...
    if duplicate:
        return {
            ...
            "resolved_semester_id": semester_context["resolved_semester_id"],
            "resolved_section_id": request.section_id,
            "duplicate": _clean_model_dict(duplicate),
        }
    ...
    logger.warning("schedule-class final insert values=%s", _clean_model_dict(new_schedule))
...
def check_duplicate(request: DuplicateRequest, db: Session = Depends(get_db)):
    """Check if a section already has a class in the same time window."""
    request_payload = _request_to_dict(request)
    semester_context = _resolve_semester_context(
        db,
        request.academic_batch_id,
    ...
    logger.warning("check-duplicate incoming payload=%s", request_payload)
    logger.warning("check-duplicate resolved semester=%s", semester_context)
    ...
    logger.warning(
        "check-duplicate result count=%s rows=%s",
        len(duplicates),
        [_clean_model_dict(row) for row in duplicates],
    )
    ...
    return {
        ...
        "resolved_semester_number": semester_context["resolved_semester_number"],
        "resolved_section_id": request.section_id,
        "duplicates": [_clean_model_dict(row) for row in duplicates]
    }
...
def map_lesson(request: MapLessonRequest, db: Session = Depends(get_db)):
    try:
        ...
        academic_batch_id = request.academic_batch_id or section.academic_batch_id
        semester_id = request.semester_id or section.semester_id
